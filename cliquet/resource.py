...
class ProtectedResource(BaseResource):
    ...
    def process_record(self, new, old=None):
        """Read permissions from request body, in order to give the ``write``
        permission on the current object for the current user
        (Making sure whose can write cannot never loose access).

        In the protocol, the ``permissions`` entry reside outside the ``data``,
        and therefore moved to new record content here.
        """
        permissions = self.request.validated.get('permissions', {})

        is_patch = (self.request.method.lower() == 'patch')

        if is_patch or (not permissions and old is not None):
            # XXX: transaction (we are reading before replacing)!
            perm_id = self.collection.get_permission_object_id(self.record_id)
            backend = self.request.registry.permission
            stored = backend.object_permissions(perm_id)
            if is_patch:
                stored.update(**permissions)
            permissions = stored

        # Always add current user to ``write``
